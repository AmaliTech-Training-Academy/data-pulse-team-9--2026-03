...
export default function DashboardOverview() {
  ...
  const rulesDefined =
    datasets.reduce((sum, d) => sum + (d.total_rules || 0), 0) || 156;
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100">
                  {datasets.slice(0, 5).map((dataset) => (
                    <tr
                      key={dataset.dataset_id || dataset.id}
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        {dataset.score !== null ? (
                          <span
                            className={`inline-flex items-center justify-center px-2.5 py-1 rounded-full text-xs font-bold ${getScoreColor(dataset.score)}`}
                          >
                            {dataset.score}%
                          </span>
                        ) : (
                      ...
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            className="p-1.5 text-gray-400 hover:text-accent bg-white rounded-md border border-gray-200 shadow-sm transition-colors"
                            title="Run Check"
                          >
                            <Play size={16} />
                          </button>
                          <button
                            className="p-1.5 text-gray-400 hover:text-primary bg-white rounded-md border border-gray-200 shadow-sm transition-colors"
                            title="View Details"
                          >
                            <Eye size={16} />
                          </button>
          ...
          <div className="bg-panel rounded-xl shadow-sm border border-gray-100">
            <div className="p-6 border-b border-gray-100">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Latest validation check results.
              </p>
            </div>
            <div className="p-0">
              {datasets
                  ...
                  >
                    <div className="flex-1 min-w-0 pr-4">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {report.checked_at
                          ? new Date(report.checked_at).toLocaleDateString()
                          : ""}
                      </p>
                    </div>
