    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const processed = datasetsArray.map((d: { id: number; name?: string; file_type?: string; uploaded_by?: { email?: string }; uploaded_at: string; status: string }) => {
                    return {
                        id: d.id,
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-gray-500">Manage, validate, and moderate datasets uploaded by all registered users.</p>
            </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[1000px]">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200">
                                ...
                                <th className="py-4 px-6 text-xs font-bold text-primary uppercase tracking-wider">Uploaded By</th>
                            ...
                            {isLoading ? (
                                <tr>
                                    <td colSpan={8} className="py-12 text-center">
                                        ...
                                        <p className="text-gray-500 font-medium">Loading system datasets...</p>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <td colSpan={8} className="py-12 text-center">
                                        ...
                                        <p className="text-gray-500">No system datasets match your criteria.</p>
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={dataset.id} className="hover:bg-[#F4F6F8]/50 transition-colors group">
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-sm font-medium text-gray-700 whitespace-nowrap">{dataset.user}</span>
                                            </div>
                                        </td>
                                        <td className="py-4 px-6 text-sm text-gray-500">{dataset.date}</td>
                                        <td className="py-4 px-6 text-sm font-medium">
                                            <span className={`flex items-center gap-1.5 ${getStatusColor(dataset.status)} whitespace-nowrap`}>
                                        ...
                                        <td className="py-4 px-6 text-right">
                                            <div className="flex items-center justify-end gap-2 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                                <button className="p-2 text-gray-400 hover:text-accent bg-white rounded-lg border border-gray-200 shadow-sm transition-colors" title="Force Run Validation">
                                                    <Play size={16} />
                                                </button>
                                                <Link href={`/dashboard/admin/datasets/${dataset.id}`} className="p-2 text-gray-400 hover:text-primary bg-white rounded-lg border border-gray-200 shadow-sm transition-colors" title="View Details">
                                                    <Eye size={16} />
                                                </Link>
                                                ...
                                                <button className="p-2 text-gray-400 hover:text-danger bg-white rounded-lg border border-gray-200 shadow-sm transition-colors" title="Delete Dataset">
                                                    <Trash2 size={16} />
                                                </button>
                                            </div>
                                        </td>
