...
export default function FileUpload() {
    ...
    const validateAndSetFile = (selectedFile: File) => {
        ...
        if (validTypes.includes(selectedFile.type) || extension === 'csv' || extension === 'json') {
    ...
    return (
        <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 w-full max-w-4xl mx-auto">
            ...
            {!file && uploadState !== "error" && (
                <div
                    className={`relative flex flex-col items-center justify-center p-12 border-2 border-dashed rounded-xl transition-colors cursor-pointer ${dragActive ? 'border-accent bg-accent/5' : 'border-gray-300 bg-gray-50 hover:bg-gray-100 hover:border-gray-400'
                        }`}
                ...
                >
                    ...
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${dragActive ? 'bg-accent/10 text-accent' : 'bg-white text-gray-400 border border-gray-200 shadow-sm'}`}>
                        <UploadCloud size={32} />
                    </div>

                    <h3 className="text-lg font-bold text-primary mb-1">Click to upload or drag and drop</h3>
                    <p className="text-sm text-gray-500 mb-6 flex items-center gap-2">
                        <FileText size={16} /> CSV or <FileJson size={16} /> JSON (max. 50MB)
                    </p>
            ...
            {uploadState === "error" && !file && (
                <div className="flex flex-col items-center justify-center p-12 border-2 border-danger/30 bg-danger/5 rounded-xl text-center">
                    ...
                    <p className="text-danger/70 mb-6">{errorMessage || "Please upload a valid CSV or JSON file."}</p>
                    <button
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between p-4 border border-accent/20 bg-accent/5 rounded-xl">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h4 className="font-semibold text-primary truncate max-w-[200px] sm:max-w-xs">{file.name}</h4>
                    ...
                    {uploadState === "success" && (
                        <div className="p-4 bg-success/10 border border-success/20 rounded-lg flex items-start gap-3 text-success">
                            ...
                            <div>
                                ...
                                <p className="text-sm mt-1 opacity-90">Your dataset has been securely uploaded and is ready for validation rules.</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        {uploadState === "success" && (
                            <>
                                ...
                                <button
                                    className="px-6 py-2.5 bg-primary text-white font-medium rounded-lg hover:bg-primary/90 shadow-lg transition-all"
                                >
